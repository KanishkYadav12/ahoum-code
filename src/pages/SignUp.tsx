...
export default function SignUp() {
	...
	const { signup, signupForm, updateSignupForm, error, isLoading, clearError } = useAuthStore();
	const [showPassword, setShowPassword] = useState(false);

	const isEmailValid = useMemo(() => isValidEmail(signupForm.email), [signupForm.email]);
		...
		<main className="flex min-h-screen flex-col bg-white px-6 pt-12 md:items-center md:justify-center md:bg-[#F8F8F8]">
			<div className="flex w-full flex-col md:max-w-[400px] md:rounded-[20px] md:bg-white md:p-10 md:shadow-lg">
				...
				<form onSubmit={handleSignUp} className="mt-10 flex flex-col md:mt-8">
					...
					<p className="mt-5 font-poppins text-[14px] leading-[22px] text-[#7C7C7C]">
						By continuing you agree to our{" "}
						<button type="button" className="text-[#53B175]">Terms of Service</button> and{" "}
						<button type="button" className="text-[#53B175]">Privacy Policy.</button>
					</p>
